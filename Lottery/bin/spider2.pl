...
open ( LOGFILE, ">>", "log2.txt" ) || die "can't open file log file\n";


#抓取已满员页面
$pageMax = getMaxNumber( getHomeFlag2( 1 ) );
OutputLog( "已满员：\$pageMax: $pageMax" );
...
sub saveHtml
{
       ...
       my ($myHtmlString) = @_;
	open( FILE, ">", "2-$fileName.htm" ) or OutputLog ( "can't open $fileName.htm: $!" );
...
#获取满员首页
sub getHomeFlag2{
	OutputLog( "program on getHomeFlag2" );
	my ($pageNumber) = @_;
	my $response = $browser->post( $url,    
				[ 'page' => $pageNumber,   
  				'lottery_type' => 'SSQ', 
